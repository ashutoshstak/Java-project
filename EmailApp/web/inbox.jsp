    ...
    <body>        
        ...
        <div id="d2" style="width:850px;background-color: aliceblue;float: right">
            <sql:query dataSource="${con}" var="rs">
                Select * from emails where tomail=? and (status='N' or status='R' ) order by emaildate
                <sql:param value="${ulogin}" />
            </sql:query>
                <table border='1'>
                    <tr>
                        <th>From</th><th>Date</th><th>Subject</th><th>Message</th><th>Status</th><th></th><th></th>
                    </tr>
                    <c:forEach items="${rs.rows}" var="row">
                        <c:if test="${row.status eq 'N'}">
                            <c:set var="clr" value="gold" />
                        </c:if>
                        <c:if test="${row.status eq 'R'}">
                            <c:set var="clr" value="green" />
                        </c:if>
                        <tr style='background-color:<c:out value="${clr}"/>'>
                            <td><c:out value="${row.frommail}" /></td>
